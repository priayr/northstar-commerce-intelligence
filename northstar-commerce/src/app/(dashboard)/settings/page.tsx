...
import { User, LogOut, Monitor, Moon, Sun, Palette, Settings as SettingsIcon, Info, Database } from "lucide-react";
import { Button } from "@/components/ui/button";
import { usePreferencesStore, type CurrencyCode, type DateFormatOption, type ComparisonPeriod, type TableDensity, type ThemeOption } from "@/store/preferences";

const SELECT_CLASS = "w-full h-10 px-3 py-2 bg-[var(--color-surface-1)] border border-[var(--color-border-strong)] rounded-md text-sm text-[var(--color-text-primary)] focus:outline-none focus:border-[var(--color-accent)] focus:ring-1 focus:ring-[var(--color-accent)]";

export default function SettingsPage() {
  // Read from global store
  const store = usePreferencesStore();

  // Local draft state so dropdowns feel responsive before saving
  const [theme, setTheme] = useState<ThemeOption>(store.theme);
  const [currency, setCurrency] = useState<CurrencyCode>(store.currency);
  const [dateFormat, setDateFormat] = useState<DateFormatOption>(store.dateFormat);
  const [comparison, setComparison] = useState<ComparisonPeriod>(store.comparison);
  const [density, setDensity] = useState<TableDensity>(store.density);
  const [saved, setSaved] = useState(false);

  // Sync local draft when store changes (e.g. navigating back to settings)
  useEffect(() => {
    setTheme(store.theme);
    setCurrency(store.currency);
    setDateFormat(store.dateFormat);
    setComparison(store.comparison);
    setDensity(store.density);
  }, []); // eslint-disable-line react-hooks/exhaustive-deps

  const handleSave = () => {
    // Persist all preferences to the Zustand store (auto-saves to localStorage)
    store.saveAll({ theme, currency, dateFormat, comparison, density });
        ...
        <div className="md:col-span-2 flex flex-col gap-6">
            ...
            <div className="grid grid-cols-3 gap-4">
              {(["light", "dark", "system"] as ThemeOption[]).map((t) => (
                <button
                  key={t}
                  onClick={() => setTheme(t)}
                  className={`flex flex-col items-center gap-3 p-4 rounded-lg border-2 transition-all ${theme === t ? "border-[var(--color-accent)] bg-[var(--color-accent)]/5" : "border-[var(--color-border)] hover:border-[var(--color-border-strong)]"}`}
                >
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                    t === "light" ? "bg-white border border-slate-200 text-slate-800" :
                    t === "dark" ? "bg-slate-900 border border-slate-700 text-white" :
                    "bg-gradient-to-br from-slate-100 to-slate-800 border border-slate-300 text-slate-500"
                  }`}>
          ...
          <div className="bg-[var(--color-surface-0)] border border-[var(--color-border)] rounded-xl p-6 shadow-sm">
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <select value={currency} onChange={(e) => setCurrency(e.target.value as CurrencyCode)} className={SELECT_CLASS}>
                ...
                <div className="space-y-2">
                  ...
                  <select value={dateFormat} onChange={(e) => setDateFormat(e.target.value as DateFormatOption)} className={SELECT_CLASS}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-[var(--color-border)]">
                <div className="space-y-2">
                  ...
                  <select value={comparison} onChange={(e) => setComparison(e.target.value as ComparisonPeriod)} className={SELECT_CLASS}>
                ...
                <div className="space-y-2">
                  ...
                  <select value={density} onChange={(e) => setDensity(e.target.value as TableDensity)} className={SELECT_CLASS}>
            ...
            <div className="mt-8 flex items-center justify-end gap-4">
              {saved && (
                <span className="text-sm font-medium text-[var(--color-positive)] animate-fade-in flex items-center gap-1">
                  ✓ Preferences saved!
                </span>
              )}
